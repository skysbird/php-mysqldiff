...
class cConnection {
	...
	function open($persistent = FALSE) {
		...
		return isset($this->_con) && trim(strval($this->_con)) != "";
	}
...
class cCommandBuilder {
	...
	*/
	function _alternateNullDefault($type) {
		if ( strtolower(substr($type, 0, 4))=="int(" || strtolower(substr($type, 0, 8))=="bigint(" || strtolower(substr($type, 0, 9))=="smallint(" || strtolower(substr($type, 0, 8))=="decimal("||strtolower(substr($type, 0, 8))=="tinyint(" || strtolower(substr($type, 0, 10))=="mediumint(" ) {
			$result="0";
